{{-- /views/waiter/orders/create.blade.php --}}
<x-app-layout>
    <x-slot name="header">
            ...
            <div class="bg-white shadow-xl sm:rounded-lg overflow-hidden">
                ...
                <form method="POST" action="{{ route('waiter.orders.store') }}">
                                ...
                                @foreach($food as $product)
                                    ...
                                    <div class="border rounded-lg p-4 flex justify-between items-center">
                                        <div class="flex items-center space-x-2">
                                            {{-- Most már a $product tartalmazza az allergéneket --}}
                                            <button type="button" @click="openModal({{ json_encode($product) }})" class="flex-shrink-0 w-6 h-6 rounded-full bg-gray-200 text-gray-600 hover:bg-gray-300 flex items-center justify-center font-bold text-sm">i</button>
                                            <div>
                                                <div class="font-medium">{{ $product->name }}</div>
                                                <div class="text-sm text-gray-600">{{ $product->price }} Ft</div>
                                            </div>
                                        </div>
                                        
                                        {{-- Mennyiség választó --}}
                                        <div class="ml-4 flex items-center space-x-2">
                                            <button type="button" x-show="getQuantity({{ $product->product_id }}) > 0" @click="removeFromCart({{ $product->product_id }})" class="bg-gold-500 text-white w-8 h-8 rounded-full text-lg font-bold hover:bg-gold-700">-</button>
                                            ...
                                            <button type="button" @click="addToCart({{ json_encode($product) }})" class="bg-gold-500 text-white w-8 h-8 rounded-full text-lg font-bold hover:bg-gold-700">+</button>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex justify-end">
                        <a href="{{ route('waiter.dashboard') }}" class="py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50">
                            Mégse
                        </a>
                        <button 
                            type="submit" 
